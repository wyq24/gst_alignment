...
def compared_arraies(fitsfile, ref_fitsfile, tar_resized=True):
    ...
    imit_shape = (int(cur_data.shape[0] * cur_map.meta['cdelt1'] / ref_map.meta['cdelt1']),
                  int(cur_data.shape[1] * cur_map.meta['cdelt2'] / ref_map.meta['cdelt2']))
...
def convert_matrix(inp_matrix):
    ...
    print(
        'Scaling factor is {}, translation is [{},{}] arcsec, rotation is {} degree(unti-clockwised) with respect to the central point of the image'.format(
            cscaling, ctranslation_xy[0], ctranslation_xy[1], crotation))
